...
from hyena_utils import (print_rank_0, 
                         column_split,
                         InferenceParams, 
...
class ParallelHyenaFilter(nn.Module):
    ...
    def forward(self, 
        ...
        if (
            ...
            y, inference_params = self.sequential_forward(u, inference_params)

        else:
            # almost always parallel forward?
            y, inference_params =  self.parallel_forward(u, inference_params, padding_mask)
        # TODO HERE IS ENTRY POINT
        ssm_state = None
        if return_ssm_state:
            # ssm_state = inference_params.fir_state_dict[self.layer_idx].cpu().detach().reshape(-1)
            ssm_state = inference_params.state_dict[self.layer_idx].cpu().detach().reshape(-1)
        return y, inference_params, ssm_state
    ...
    def sequential_forward(self, u, inference_params):
        print("sequential forward at layer ", self.layer_idx)
...
class ParallelGatedConvBlock(nn.Module):
    def __init__(self, 
        ...
        # self.return_iir_state = return_iir_state 
        self.filter = ParallelHyenaFilter(config, 
                                          layer_idx,).to(dtype=dtype)
    ...
    def forward(self, u, inference_params=None, padding_mask=None, return_ssm_state=False, *args, **kwargs):
        z = self.projections(self.pre_norm(u))
        if type(padding_mask) == torch.Tensor:  # guard against bias
            z = z * padding_mask[..., None]

        z, inference_params, ssm_state = self.filter(
            z, 
            inference_params=inference_params, 
            padding_mask=padding_mask,
            return_ssm_state=return_ssm_state,
        )
        ...
        if type(padding_mask) == torch.Tensor:  # guard against bias
            u = u * padding_mask[..., None]
        u = self.mlp(self.post_norm(u)) + u
        return u, inference_params, ssm_state
...
def get_block(config, 
    ...
    if layer_idx in config.attn_layer_idxs:
        ...
        block = ParallelGatedConvBlock(config, 
                                       layer_idx,)
...
class StripedHyena(nn.Module):
    def __init__(self, config,):
        ...
        self.embed_layer_idx = config.ssm_layer_idx
        ...
        iir_state_flags = [False for i in range(config.num_layers)]
        iir_state_flags[self.embed_layer_idx] = True 

        self.blocks = nn.ModuleList(
            get_block(config, 
                      layer_idx, 
                      return_iir_state = flag,
                      flash_fft=self.flash_fft)
            for layer_idx, flag in enumerate(iir_state_flags)
        )
    #TODO: reshape
    def forward(self, x, inference_params_dict=None, padding_mask=None, ssm_layer_idx=None):
        L = x.shape[1]
        x = self.embedding_layer.embed(x)
        iir_state = None 
        if inference_params_dict is not None:
            # has to be in stateful forward
            x, inference_params_dict_out, iir_state = self.stateful_forward(
                x,
                inference_params_dict=inference_params_dict,
                ssm_layer_idx=ssm_layer_idx,
            )
        else:
            x, inference_params_dict_out = self.stateless_forward(x, padding_mask=padding_mask)
        x = self.norm(x)
        x = self.unembed.unembed(x)
        return x, inference_params_dict_out, iir_state

    def stateful_forward(self, x, inference_params_dict=None,ssm_layer_idx=None):
        ssm_state = None
        if ssm_layer_idx is not None:
            assert ssm_layer_idx >= 0, "invalid ssm_layer_idx"
        for block_idx, block in enumerate(self.blocks):
            ...
            if block_idx == ssm_layer_idx:
                x, _, ssm_state = block(x, 
                                        inference_params=inference_params,
                                        return_ssm_state=True)
            else:
                x, _, _ = block(x,inference_params=inference_params,retrun_iir_state=False)
            # if block.return_iir_state and block_name == "mha":
            #     raise ValueError("state vector should not be returned for MHA blocks")
        return x, inference_params_dict, ssm_state
